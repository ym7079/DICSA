...
class Experiment {
    ...
    private int HostNestNum;
    private double Pa;
    private double alpha;

    ArrayList<Solution_SR> HostNest = new ArrayList<>();
    ArrayList<Solution_DR> HostNest_DR = new ArrayList<>();

    Solution_SR currBestCuckoo;
...
// 생성자
    Experiment(int HostNestNum, double Pa, double alpha){
        this.numDepart = ProbDataSet.getNumDepart();
...
// Method : The 1st phase -> Generate initial population of n host nests
    void initHostNest() throws Exception { // 완전 랜덤한 초기해 생성
        ...
        currBestCuckoo_DR = HostNest_DR.get(0);
        bestIdx = 0;
    }
    void initHostNest(String layoutProbType) throws Exception {
        ...
        if(layoutProbType == "SR"){
            HostNest.clear();
            HostNest.add(new Solution_SR(Π,g.keySeqUpdate(Π)));
        ...
        for(int i=1; i<HostNestNum; i++){
            ...
            if(layoutProbType == "SR"){
                HostNest.add(new Solution_SR(tmpDepartSeq,g.keySeqUpdate(tmpDepartSeq)));
                Collections.sort(HostNest);
                currBestCuckoo = HostNest.get(0);
            }
            else if(layoutProbType == "DR"){
    ...
    void initHostNest(Solution_DR currBestCuckoo) throws Exception {
        ...
        // HostNest의 초기화
        HostNest_DR.clear();
        //HostNest.add(new Solution(Π,g.keySeqUpdate(Π)));
        for(int i=0; i<HostNestNum/2; i++){
            LinkedList<Integer> tmpDepartSeq = (LinkedList<Integer>) Π.clone();
            for(int j=0; j<=numDepart/2; j++){
                double rn = Math.random();
                if(rn <= 0.5){
                    Collections.swap(tmpDepartSeq,j,numDepart-j-1);
...
// Method : The 2nd phase -> Triggering smart cuckoos
    ...
    void searchWithSmartCuckoos_SR() throws Exception {
        ...
        while(true){
            ...
            generateNewSolutionByLévyFlights(HostNest.get(j));
            newCuckoo = new Solution_SR(newDepartSeq,newKeySeq);

            // Prevent Overlap Solution
            boolean sameSolution = preventOverlap(newCuckoo);
            if (!(sameSolution)) {
                if(newCuckoo.compareTo(HostNest.get(j)) <= -1) {
                    HostNest.set(j, newCuckoo);//==> ofv 값이 안 좋더라도 무조건 update
                    if (newCuckoo.compareTo(HostNest.get(bestIdx)) <= -1) {
                        bestIdx = j;//--> population 에서 ofv 가 가장 좋은 solution 보다 더 좋으면
                        if (newCuckoo.compareTo(currBestCuckoo) <= -1) {
                            currBestCuckoo = newCuckoo;
                            System.out.print(currBestCuckoo.departSeq+"\t");
                            System.out.println("best OFV update_s : "+ currBestCuckoo.OFV+"\tIteration: "+ RK_Main.numIter);
                            currBestCuckoo.iterNum = RK_Main.numIter;
                            RK_Main.numIter = 0;
                        }
                    }
                }
...
// Method : The 3rd phase -> Employ one cuckoo to search for a new good solution
    Solution_SR getCuckooI() throws Exception {
        Solution_SR newCuckoo;
        while(true) {
            generateNewSolutionByLévyFlights(HostNest.get(bestIdx));
            newCuckoo = new Solution_SR(newDepartSeq, newKeySeq);
            // Prevent Overlap Solution
            boolean sameSolution = preventOverlap(newCuckoo);
            if (!(sameSolution)) {
    ...
    void employOneCuckoo_SR() throws Exception {
        ...
        nestJ = HostNest.get(j);
        //Compare both solutions and Update the best one
        if(newCuckoo.compareTo(nestJ) <= -1) {
            //Replace j by i(new one)
            HostNest.set(j, newCuckoo);
            if (newCuckoo.compareTo(HostNest.get(bestIdx)) <= -1){
                bestIdx = j;
                if (newCuckoo.compareTo(currBestCuckoo) <= -1) {
                    System.out.print(newCuckoo.departSeq+"\t");
                    currBestCuckoo = new Solution_SR(newCuckoo.departSeq,newCuckoo.keySeq);
                    System.out.println("best OFV update_b : "+ currBestCuckoo.OFV+"\tIteration: "+ RK_Main.numIter);
                    currBestCuckoo.iterNum = RK_Main.numIter;
                    RK_Main.numIter = 0;
                }
            }
            if(Pa != 0.0) {
                Collections.sort(HostNest); // Nest j 가 업데이트 되었으므로 정렬
                bestIdx = 0;
            }
        }
    }
    ...
    void abandonWorseNests_SR() throws Exception {
        ...
        for (int i=0; i< numAbandonNest; i++) {
            ...
            newCuckoo = replaceNest(HostNest.get(bestIdx));
            HostNest.set(worstIndex, newCuckoo);

            if (newCuckoo.compareTo(HostNest.get(i)) <= -1){
                bestIdx = worstIndex;
                if (newCuckoo.compareTo(currBestCuckoo) <= -1) {
                    System.out.print(newCuckoo.departSeq+"\t");
                    currBestCuckoo =  (Solution_SR) newCuckoo.clone();
                    System.out.println("best OFV update_w : "+ currBestCuckoo.OFV+"\tIteration: "+ RK_Main.numIter);
                    currBestCuckoo.iterNum = RK_Main.numIter;
                    RK_Main.numIter = 0;
                }
            }
    ...
    void abandonWorseNests_DR() throws Exception {
        ...
        for (int i=0; i< numAbandonNest; i++) {
            //Find the current worst nest & Replace some of the worst nests by constructing new solutions/nests
            int worstIndex = HostNestNum-i-1;
            newCuckoo_DR = replaceNest(HostNest_DR.get(worstIndex));
...
// Method : Get random walk Value
    private static double lambda = 3.0/2.0; // λ
    private static double sigmaU = Math.pow(((gamma(1.0+lambda) * Math.sin((Math.PI*lambda)/2.0)) / (gamma((1.0+lambda)/2.0) * lambda * Math.pow(2.0,(lambda-1)/2))),(1/lambda));
    private static double sigmaV = 1.0;
    private static double normal(){ return new Random().nextGaussian(); }
    ...
    private double stepValue(){
        ...
        double u = new Random().nextGaussian()*sigmaU;
        double v = new Random().nextGaussian()*sigmaV;

        // generate stepSize = ( u~N(0, sigma) / v~N(0, 1)^(1/lambda) )
        // stepSize = u / (|v|)^(1/Lambda)
        step = u / Math.pow((Math.abs(v)),(1.0/lambda));
        //step = normal()*(u / Math.pow((Math.abs(v)),(1/lambda))); => 수민 closedLoop 논문 version

        return step;
    } //s is step size drawn from a Lévy distribution.
    private double Levy(double s, double λ){ return 1.0/(Math.pow(Math.abs(s),λ));}
...
// Method : Replace some nests by constructing new solutions/nests using Lévy flights (The local random walk) /////////////////////////
    ...
    private Solution_DR replaceNest(Solution_DR worstNest) throws Exception {
        Solution_DR newCuckoo;
        while (true){
            /*newDepartSeq = (LinkedList<Integer>) worstNest.departSeq.clone();
            g.bigJumpOperator(newDepartSeq);
            g.bigJumpOperator(newDepartSeq);
            newKeySeq = (LinkedList<Double>) g.normalizedKeySeq.clone();
